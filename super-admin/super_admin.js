...
async function openTenantMasterModal(slug) {
    ...
    // 7 Extended Controls
    document.getElementById('master-timezone').value = tenant.timezone || config.timezone || 'Africa/Lagos';
    document.getElementById('master-workdays').value = tenant.workdays || config.workdays || '1_5';

    const lateCutoff = tenant.late_cutoff_minutes !== undefined ? tenant.late_cutoff_minutes : (config.late_cutoff_minutes !== undefined ? config.late_cutoff_minutes : 510);
    const lateH = String(Math.floor(lateCutoff / 60)).padStart(2, '0');
    const lateM = String(lateCutoff % 60).padStart(2, '0');
    document.getElementById('master-late-cutoff').value = `${lateH}:${lateM}`;

    const closingMin = tenant.workday_end_minutes !== undefined ? tenant.workday_end_minutes : (config.workday_end_minutes !== undefined ? config.workday_end_minutes : 1020);
    const closeH = String(Math.floor(closingMin / 60)).padStart(2, '0');
    const closeM = String(closingMin % 60).padStart(2, '0');
    document.getElementById('master-closing-time').value = `${closeH}:${closeM}`;

    const wfhEnabled = tenant.wfh_quota_enabled !== undefined ? tenant.wfh_quota_enabled : (config.wfh_quota_enabled !== undefined ? config.wfh_quota_enabled : true);
    document.getElementById('master-wfh-quota').value = String(wfhEnabled);

    const leadPriority = tenant.team_lead_priority_sort !== undefined ? tenant.team_lead_priority_sort : (config.team_lead_priority_sort !== undefined ? config.team_lead_priority_sort : true);
    document.getElementById('master-lead-priority').value = String(leadPriority);

    document.getElementById('master-hybrid-office-days').value = String(tenant.hybrid_office_days || config.hybrid_office_days || 2);
...
async function handleSaveMasterPolicies(e) {
    ...
    const timezone = document.getElementById('master-timezone').value;
    const [lateH, lateM] = (document.getElementById('master-late-cutoff').value || '08:30').split(':').map(Number);
    const late_cutoff_minutes = (lateH * 60) + (lateM || 0);

    const [closeH, closeM] = (document.getElementById('master-closing-time').value || '17:00').split(':').map(Number);
    const workday_end_minutes = (closeH * 60) + (closeM || 0);

    const wfh_quota_enabled = document.getElementById('master-wfh-quota').value === 'true';
    const workdays = document.getElementById('master-workdays').value;
    const team_lead_priority_sort = document.getElementById('master-lead-priority').value === 'true';
    const hybrid_office_days = parseInt(document.getElementById('master-hybrid-office-days').value, 10) || 2;
        ...
        return `
            <tr>
                <td>
                    ...
                    ${s.is_team_lead ? '<span class="staff-lead-star" title="Team Lead" data-tooltip="Team Lead" style="color:#d97706; font-size:0.95rem; margin-left:4px; vertical-align:middle;">★</span>' : ''}
                </td>
